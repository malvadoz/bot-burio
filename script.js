...
client.on("ready", () => {
    ...
    client.user.setActivity({ name: "Exotic !", type: "STREAMING", url: "https://www.twitch.tv/exotic000001" })
    Main();
});
        ...
        switch (answer) {
            ...
            case "2":
                readline.question("\n[!] Enter Guild ID: ", response => {
                    ScrapeUsers(response).then(() => {
                        setTimeout(() => {
                            readline.question("\n[i] Set Timeout: The number of seconds the bot waits before it messages users.\n[i] Bypass: Avoids being flagged by Discord\n[i] Limit(s): 3 - 25 seconds\n\n[!] Enter Timeout: ", timeout => {
                                if (timeout === "3" || timeout === "4" || timeout === "5" || timeout === "6" || timeout === "7" || timeout === "8" || timeout === "9"| timeout === "10"| timeout === "11"| timeout === "12"| timeout === "13"| timeout === "15"| timeout === "16"| timeout === "17"| timeout === "18"| timeout === "19"| timeout === "20"| timeout === "21"| timeout === "22"| timeout === "23"| timeout === "24"| timeout === "25") {
